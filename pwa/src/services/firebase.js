...
const firebasePaths = {
  ...
  /**
   * Dashboard path for a specific SPPG (used in drill-down).
   * @param {number|string} sppgId
   */
  dashboardSPPGDetail(sppgId) {
    return `/dashboard/kepala_sppg/${sppgId}`
  },

  /**
   * Get all SPPG monitoring paths under a Yayasan.
   * Used by kepala_yayasan to listen to all SPPG data.
   * @param {Array<number>} sppgIds - List of SPPG IDs under the Yayasan
   * @param {string} date
   * @returns {Array<string>} Array of monitoring paths
   */
  monitoringForYayasan(sppgIds, date) {
    return (sppgIds || []).map(id => `/monitoring/${id}/${date}`)
  },

  /**
   * Get all KDS cooking paths under a Yayasan.
   * @param {Array<number>} sppgIds
   * @param {string} date
   * @returns {Array<string>}
   */
  kdsCookingForYayasan(sppgIds, date) {
    return (sppgIds || []).map(id => `/kds/cooking/${id}/${date}`)
  }
}
